...
- **Score par classificateur** : Chaque vignette porte trois métriques de l'ensemble de test — l'**exactitude**, le **rappel** et la **précision** — sous leur libellé français, suivi du nom anglais en gris (`accuracy`, `recall`, `precision`) et de la valeur. Le survol d'une métrique en rappelle la définition. Les lire ensemble instruit, aucune ne suffisant seule : un SVM linéaire sur des lunes bruitées range presque tout dans la classe positive, d'où un rappel parfait (1.00) que démentent une exactitude de 0.42 et une précision de 0.38.
